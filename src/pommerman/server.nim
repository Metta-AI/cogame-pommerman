...
const
  ...
  ClientAssets = [
    ...
    ("/client/bomber_red.png", staticRead("../../data/bomber_red.png")),
    ("/client/bomber_blue.png", staticRead("../../data/bomber_blue.png")),
    ("/client/bomber_red_crown.png",
      staticRead("../../data/bomber_red_crown.png")),
    ("/client/bomber_blue_crown.png",
      staticRead("../../data/bomber_blue_crown.png")),
    ("/client/soldier_red.png", staticRead("../../data/soldier_red.png")),
    ("/client/soldier_blue.png", staticRead("../../data/soldier_blue.png")),
    ("/client/bomb.png", staticRead("../../data/bomb.png")),
    ("/client/powerup_range.png", staticRead("../../data/powerup_range.png")),
    ("/client/powerup_kick.png", staticRead("../../data/powerup_kick.png")),
  ...
  if request.path == PlayerClientPath and request.httpMethod == "GET":
    ...
    if not ok:
      request.respond(403, textHeaders("text/html; charset=utf-8"),
        "<!doctype html><meta charset=utf-8><title>pommerman seat</title>" &
    ...
    ## would seat the certifier's probe as a bomber (lantern 0.1.1).
    request.respond(200, textHeaders("text/html; charset=utf-8"),
      "<!doctype html><meta charset=utf-8><title>pommerman seat " &
      $slot & "</title><body style=\"font:14px system-ui;background:#16110d;" &
      "color:#f2e8d8\"><h1>pommerman</h1><p>Seat " & $slot &
      " is authorised. This seat is driven by /bin/pommerman-player over " &
      PlayerWsPath & "?slot=" & $slot & "&amp;token=...</p>")
    return
  if request.path == GlobalClientPath and request.httpMethod == "GET":
    request.respond(200, textHeaders("text/html; charset=utf-8"),
      "<!doctype html><meta charset=utf-8><title>pommerman spectator" &
      "</title><body style=\"font:14px system-ui;background:#16110d;" &
      "color:#f2e8d8\"><h1>pommerman</h1><p>The hosted spectator " &
  ...
  if request.httpMethod == "GET":
    for (path, art) in ClientAssets:
      if request.path == path:
        ...
        return
  request.respondText(200, "pommerman server\n")
...
proc declarePlayerFailure(slot: int, message: string) =
  ...
  ## nothing else. Best-effort: a declaration write failure must never mask the
  ## episode.
  try:
    writeCogameEnv(
...
proc runServerLoop*(
  ...
  echo "pommerman listening on ", host, ":", port,
    " board=", BoardSize, "x", BoardSize, " seats=", config.numAgents,
  ...
  while true:
          ...
          if not sim.joined[seat.slot]:
            ...
            writer.writeJoin(frame, seat.slot,
              (if sim.seatNames[seat.slot].len > 0: sim.seatNames[seat.slot]
               else: seatAliasOf(seat.slot)), seat.token)
    ...
    for (slot, text) in newRegistrations:
      ...
      if firstRegistration:
        ...
        echo "seat ", slot, " (", seatAliasOf(slot), ") registered: kind=",
          engine.policyKind(slot), " baseline=", $engine.seats[slot].baseline
      ...
      if episodeFrame.startedGame:
        if driver.failureSlot >= 0 and not failureDeclared:
          ...
          declarePlayerFailure(driver.failureSlot,
            ...
            " lobby ticks; its bomber plays the sapper baseline")
        echo "game starts: 4 bombers, ", config.maxTicks, " ticks, ",
          sim.turnsPerGame(), " command turns"
      if episodeFrame.finishedGame:
        echo "game done: ", sim.gameLog[^1].endRule, " alive ",
          sim.gameLog[^1].alive[TeamRed], " v ", sim.gameLog[^1].alive[TeamBlue]
      if episodeFrame.faulted:
        echo "pommerman: HOST ERROR at tick ", sim.tick, ": ", sim.stopDetail
      if sim.collectEvents:
        for event in sim.events:
          collectedEvents.add(event)
        sim.events.setLen(0)
      if driver.stopped:
        echo "pommerman: ", sim.stopDetail, "; settling from this tick"
      if driver.finished:
        quitAfterFrame = true
  ...
  if not replayMode:
    ...
    let resultsJson = sim.bomberResultsJson() & "\n"
