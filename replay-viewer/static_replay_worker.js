...
function stageNote() {
  ...
  try {
    var length = Module._pom_stage_len ? Module._pom_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._pom_stage_ptr();
...
function runtimeError() {
  var length = Module._pom_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._pom_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._pom_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._pom_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._pom_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._pom_load_replay(pointer, length);
    });
    ...
    postMessage({
      type: 'loaded',
      mismatchTick: Module._pom_mismatch_tick()
    });
  } catch (error) {
    reportFailure(error);
  }
...
function applyInputNow() {
  ...
  try {
    if (Module._pom_frame() < 0) throw new Error(runtimeError());
    ingestPacket();
    postMessage({
      type: 'inputApplied',
      mismatchTick: Module._pom_mismatch_tick()
